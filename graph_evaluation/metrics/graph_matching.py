...
def get_bert_score(all_gold_edges, all_pred_edges):
    ...
    _, _, bs_F1 = score_bert(cands=candidates, refs=references, model_type="bert-base-uncased", lang='en', idf=False, device="cuda:0")
...
def get_ged(gold_graph, pred_graph=None):
    ...
    for edge in pred_graph[:len(gold_graph)]:
        if len(edge) == 2:
            edge.append('NULL')
        elif len(edge) == 1:
            edge.append('NULL')
